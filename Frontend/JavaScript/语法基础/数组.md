...
:warning: 数组可以使用语法 `new Array([number])` 创建，但很少被使用因为方括号 `[]` 更短更简洁。而且这种语法还存在一些诡异的特性，如果为构造函数传递参数（即数字）会创建一个 **指定了长度，却没有任何项** 的数组。为了避免这种乌龙事件，我们通常都是使用方括号的，除非我们清楚地知道自己正在做什么。
...
alert( fruits ); // Banana, Pear — 现在有 2
```
...
:warning: 数组默认不支持负数作为索引值，可使用 `array.length - n` 的方法来访问倒数第 `n` 个元素
...
:bulb: `length` 属性是可写的，如果我们减少它，数组就会被截断，且该过程是不可逆的，利用该特性可以快速清空数组 `arr.length = 0`
...
此外数组还可以作为[栈 stack](https://en.wikipedia.org/wiki/Stack_(abstract_data_type))
...
:bulb: 不带参数地调用该方法 `arr.slice()` 会创建一个数组副本，以进行不影响原始数组的进一步转换。
...
:bulb: 通常它只复制数组中的元素。其他对象，即使类数组的对象，仍然会被作为一个整体添加；但如果类似数组的对象具有 `Symbol.isConcatSpreadable` 属性，那么它就会被当作一个数组来处理，即对象中的数字作为关键字 `key` 的属性将会将被整合。
...
- 方法 `arr.indexOf(item, from)` 从索引 `from` 开始搜索 `item`，如果找到则返回索引，**否则返回 `-1`**。
- 方法 `arr.lastIndexOf(item, from)` 从右向左搜索。
- 方法 `arr.includes(item, from)` 从索引 `from` 开始搜索 `item`，如果找到则返回 `true`；如果没找到，则返回 `false`。

:bulb: 如果只想检查是否包含某个元素，并且不想知道确切的索引，方法 `arr.includes()` 是首选。

:bulb: 以上方法使用严格相等 `===` 比较，但 `include()` 可以正确处理 `NaN` 情况（一般 `NaN` 无法通过比较进行匹配，因为[它与自身也不相等](D:/Front_end/JavaScript/number类型.md#数字特殊值)）
...
:warning: **默认将元素转换为字符串再进行排序**，按照各个字符的 Unicode 位点进行排序，可能造成数字次序排序问题。因此进行数字排序时应该使用比较函数
...
* 如果 `compareFunction(a, b)` 返回值负数表示 `a` 小于 `b`，则 `a` 会被排列到 `b` 之后
...
#### reverse()
方法 `reverse()` 返回方向的数组，即数组的第一个元素会变成最后一个，数组的最后一个元素变成第一个。
